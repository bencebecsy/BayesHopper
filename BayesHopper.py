...
Trans-dimensional Bayesian sampler for gravitational-wave data analysis
...
def run_ptmcmc(N, T_max, n_chain, pulsars, max_n_source=1,
               n_source_prior='flat', n_source_start='random', RJ_weight=0,
               regular_weight=3, noise_jump_weight=3, PT_swap_weight=1,
               T_ladder=None, Fe_proposal_weight=0, fe_file=None,
               Fe_pdet=0.5, Fe_alpha=0.1,
               prior_recovery=False, cw_amp_prior='uniform',
               gwb_amp_prior='uniform', rn_amp_prior='uniform',
               per_psr_rn_amp_prior='uniform', gwb_log_amp_range=[-18,-11],
               n_comp_common=30, n_comp_per_psr_rn=30,
               rn_log_amp_range=[-18,-11], per_psr_rn_log_amp_range=[-18,-11],
               ...
               include_rn=False, include_per_psr_rn=False, vary_rn=False,
               vary_per_psr_rn=False, rn_params=[-13.0,1.0],
               per_psr_rn_start_file=None, rn_on_prior=0.5, rn_switch_weight=0,
               jupyter_notebook=False, gwb_on_prior=0.5,
               include_equad_ecorr=False, wn_backend_selection=False,
               noisedict_file=None, save_every_n=10000, savefile=None,
               rn_gwb_move_weight=0):

    ptas = get_ptas(pulsars, vary_white_noise=vary_white_noise,
                    include_equad_ecorr=include_equad_ecorr,
                    ...
                    vary_rn=vary_rn, vary_per_psr_rn=vary_per_psr_rn,
                    include_gwb=include_gwb, max_n_source=max_n_source,
                    efac_start=efac_start, rn_amp_prior=rn_amp_prior,
                    ...
                    rn_params=rn_params, gwb_amp_prior=gwb_amp_prior,
                    gwb_log_amp_range=gwb_log_amp_range,
                    n_comp_common=n_comp_common,
                    n_comp_per_psr_rn=n_comp_per_psr_rn,
                    vary_gwb_gamma=vary_gwb_gamma, vary_rn_gamma=vary_rn_gamma,
                    cw_amp_prior=cw_amp_prior,
 ...
 Temperature ladder is:\n".format(n_chain,c),Ts)
    ...
    #printitng out the prior used on RN on/off
    if include_rn:
        print("Prior on RN on/off: {0}%".format(rn_on_prior*100))
    ...
    for i in range(max_n_source+1):
        ...
        for j in range(1+int(include_gwb)):
            ...
            for k in range(1+int(include_rn)):
                pimap_element = {}
                p_mask_element = []
                for idx, p in enumerate(ptas[i][j][k].param_names):
                    pimap_element[p] = idx
                    p_mask_element.append(param_names.index(p))
                pimap_sublayer.append(pimap_element)
                p_mask_sublayer.append(p_mask_element)

                print(i, j, k)
                print(ptas[i][j][k].params)
            pimap_layer.append(pimap_sublayer)
            p_mask_layer.append(p_mask_sublayer)
        pimap.append(pimap_layer)
        p_mask.append(p_mask_layer)
    ...
    for j in range(n_chain):
        #choose # of sources to start with
        ...
        print(n_source)
        #choose if GWB is on or off at start
        gwb_on = np.random.choice(2)
        samples[j,0,1] = gwb_on
        #choose if RN is on or off at start
        rn_on = np.random.choice(2)
        samples[j,0,2] = rn_on
    ...
    #rows: proposal type (RJ_CW, gwb_switch, rn_switch, rn_gwb, PT, Fe, fisher, noise_jump)
    ...
    if Fe_proposal_weight+RJ_weight>0:
        ...
        if Fe_proposal_weight>0:
            ...
            def psi_pdf(x):
                if psi_bin_edges[0] < x < psi_bin_edges[-1]:
                    bin_idx = (np.abs(psi_bin_centers - x)).argmin()
                    return psi_hist[bin_idx]
                else:
                    return 0.0
            #inc
            inc_hist, inc_bin_edges = np.histogram(np.cos(inc_max.flatten()),
            ...
            def cos_inc_pdf(x):
                if inc_bin_edges[0] < x < inc_bin_edges[-1]:
                    bin_idx = (np.abs(inc_bin_centers - x)).argmin()
                    return inc_hist[bin_idx]
                else:
                    return 0.0
            #phase0
            phase0_hist, phase0_bin_edges = np.histogram(phase0_max.flatten(),
                                                         bins=100, density=True)
            phase0_bin_centers = []
    ...
    total_weight = (regular_weight + PT_swap_weight + Fe_proposal_weight +
                    ...
                    rn_switch_weight + rn_gwb_move_weight)
...
Noise jump: {7:.2f}%".format(swap_probability*100, fe_proposal_probability*100,
                                       ...
                                       rn_gwb_move_probability*100,
                                       noise_jump_probability*100))

    for i in range(int(N-1)):
        #print(samples[0,i,:])
        #write results to file
        if savefile is not None and i%save_every_n==0 and i!=0:
            np.savez(savefile, samples=samples[:,:i,:],
                     acc_fraction=acc_fraction, swap_record=swap_record)
        ...
        if i%n_status_update==0:
            ...
            if jupyter_notebook:
                ...
                print('Progress: {0:2.2f}% '.format(i/N*100) +
                      'Acceptance fraction #columns: chain number; rows: proposal type \
                      (RJ_CW, gwb_switch, rn_switch, rn_gwb, PT, Fe, fisher, noise_jump):')
                print(acc_fraction)
        ...
        if i%n_fish_update==0 and i>=n_global_first:
            ...
            if i%(n_fish_update*10)==0:
                for j in range(n_chain):
                    ...
                    if include_gwb:
                        if vary_gwb_gamma and vary_rn_gamma:
                            dim = 4
                        elif ((not vary_gwb_gamma and vary_rn_gamma) or
                              (vary_gwb_gamma and not vary_rn_gamma)):
                            dim = 3
                        else:
                            dim = 2
                    else:
                        if vary_rn_gamma:
                            ...
                            dim = 1
                    eigvec_rn = get_fisher_eigenvectors(np.delete(samples[j,i,1:], range(n_source*7,max_n_source*7)),
                                                        ptas[n_source][gwb_on][1],
                                                        T_chain=Ts[j], n_source=1, dim=dim,
                                                        offset=n_source*7+num_per_psr_params)
                    if np.all(eigvec_rn):
                        eig_gwb_rn[j,:,:] = eigvec_rn[0,:,:]

                    #CW eigenvectors
                    if n_source!=0:
                        eigenvectors = get_fisher_eigenvectors(np.delete(samples[j,i,1:], range(n_source*7,max_n_source*7)),
                                                               ptas[n_source][gwb_on][1],
                                                               T_chain=Ts[j], n_source=n_source)
                ...
                if include_gwb:
                    ...
                    gwb_on = 0
                eigenvectors = get_fisher_eigenvectors(np.delete(samples[0,i,1:], range(n_source*7,max_n_source*7)),
                                                       ptas[n_source][gwb_on][1],
                                                       T_chain=Ts[0], n_source=n_source)
        ...
        if i<n_global_first:
            do_fe_global_jump(n_chain, max_n_source, ptas, samples, i, Ts,
                              a_yes, a_no, freqs, fe, inc_max, psi_max, phase0_max,
                              h_max, vary_white_noise, include_gwb,
                              num_noise_params, num_per_psr_params,
                              Fe_pdet, Fe_alpha,
                              psi_pdf, cos_inc_pdf, phase0_pdf, log_h_pdf)
        else:
            #draw a random number to decide which jump to do
            jump_decide = np.random.uniform()
            #PT swap move
            if jump_decide<swap_probability:
                do_pt_swap(n_chain, max_n_source, ptas, samples, i, Ts,
                           a_yes, a_no, swap_record, vary_white_noise, include_gwb,
                           num_noise_params, num_per_psr_params)
            ...
            #do RJ move
                  ...
                  RJ_probability):
                do_rj_move(n_chain, max_n_source, n_source_prior, ptas, samples, i, Ts,
                           a_yes, a_no, freqs, fe, inc_max, psi_max, phase0_max,
                           h_max, rj_record, vary_white_noise, include_gwb,
                           num_noise_params, num_per_psr_params)
            #do GWB switch move
            elif (jump_decide<swap_probability+fe_proposal_probability+
                  RJ_probability+gwb_switch_probability):
                gwb_switch_move(n_chain, max_n_source, ptas, samples, i, Ts,
                                a_yes, a_no, vary_white_noise, include_gwb,
                                num_noise_params, num_per_psr_params,
                                gwb_on_prior, gwb_log_amp_range, vary_gwb_gamma)
            #do noise jump
            elif (jump_decide<swap_probability+fe_proposal_probability+
                  RJ_probability+gwb_switch_probability+noise_jump_probability):
                noise_jump(n_chain, max_n_source, ptas, samples, i, Ts,
                           a_yes, a_no, eig_per_psr, include_gwb,
                           num_noise_params, num_per_psr_params, vary_white_noise)
            #do RN switch move
            elif (jump_decide<swap_probability+fe_proposal_probability+
                  RJ_probability+gwb_switch_probability+noise_jump_probability+
                  rn_switch_probability):
                rn_switch_move(n_chain, max_n_source, ptas, samples, i, Ts,
                               a_yes, a_no, vary_white_noise, include_rn,
                               num_noise_params, num_per_psr_params,
                               rn_on_prior, rn_log_amp_range, vary_rn_gamma)
            #do RN-GWB move
            elif (jump_decide<swap_probability+fe_proposal_probability+
                  RJ_probability+gwb_switch_probability+noise_jump_probability+
                  rn_switch_probability+rn_gwb_move_probability):
                rn_gwb_move(n_chain, max_n_source, ptas, samples, i, Ts,
                            a_yes, a_no, vary_white_noise, include_rn, include_gwb,
                            num_noise_params, num_per_psr_params,
                            rn_on_prior, rn_log_amp_range, gwb_on_prior, gwb_log_amp_range,
                            vary_rn_gamma, vary_gwb_gamma)
            #regular step
            else:
                regular_jump(n_chain, max_n_source, ptas, samples, i, Ts,
                             a_yes, a_no, eig, eig_gwb_rn, include_gwb,
                             num_noise_params, num_per_psr_params,
                             vary_rn, vary_rn_gamma, vary_gwb_gamma)

    acc_fraction = a_yes/(a_no+a_yes)
    return samples, acc_fraction, swap_record, rj_record, param_names
...
#
###############################################################################
def rn_gwb_move(n_chain, max_n_source, ptas, samples, i, Ts, a_yes, a_no,
                vary_white_noise, include_rn, include_gwb,
                num_noise_params, num_per_psr_params,
                rn_on_prior, rn_log_amp_range, gwb_on_prior, gwb_log_amp_range,
                vary_rn_gamma, vary_gwb_gamma):
    ...
    for j in range(n_chain):
        ...
        #Both are on -- miximng move
            ...
            #draw randomly exchange fraction,
            #i.e. fraction of GWB to add to RN (if positive)
            #or fraction of RN to add to GWB (if negative)
        ...
        #Switching from GWB to RN
            ...
            log_acc_ratio = ptas[n_source][0][1].get_lnlikelihood(new_point)/Ts[j]
        ...
        #Switching from RN to GWB
            ...
            log_acc_ratio = ptas[n_source][1][0].get_lnlikelihood(np.delete(new_point, range(n_source*7+num_per_psr_params, n_source*7+num_noise_params)))/Ts[j]
...
#
###############################################################################
def rn_switch_move(n_chain, max_n_source, ptas, samples, i, Ts, a_yes, a_no,
                   vary_white_noise, include_rn,
                   num_noise_params, num_per_psr_params,
                   rn_on_prior, rn_log_amp_range, vary_rn_gamma):
    if not include_rn:
    ...
    for j in range(n_chain):
        ...
        if rn_on==1:
            ...
            if vary_rn_gamma:
                ...
                old_log_amp = np.copy(new_point[n_source*7+num_per_psr_params])
            #make a dummy enterprise parameter which we will use for getting
            #the proposal density at the value of the old amplitude
            sampling_parameter = parameter.Uniform(rn_log_amp_range[0],
                                                   rn_log_amp_range[1])('dummy')
            ...
            log_acc_ratio = ptas[n_source][gwb_on][0].get_lnlikelihood(np.delete(new_point, range(n_source*7+num_per_psr_params, n_source*7+num_noise_params)))/Ts[j]
        ...
        #turning on -----------------------------------------------------------
            ...
            #draw new amplitude
            #make a dummy enterprise parameter which we will use for
            #drawing from a log-uniform A distribution
            sampling_parameter = parameter.Uniform(rn_log_amp_range[0],
                                                   rn_log_amp_range[1])('dummy')
...
#
###############################################################################
def gwb_switch_move(n_chain, max_n_source, ptas, samples, i, Ts, a_yes, a_no,
                    vary_white_noise, include_gwb,
                    num_noise_params, num_per_psr_params,
                    gwb_on_prior, gwb_log_amp_range, vary_gwb_gamma):
    if not include_gwb:
    ...
    for j in range(n_chain):
        ...
        if gwb_on==1:
            ...
            #make a dummy enterprise parameter which we will use for getting
            #the proposal density at the value of the old amplitude
            sampling_parameter = parameter.Uniform(gwb_log_amp_range[0],
                                                   gwb_log_amp_range[1])('dummy')
            ...
            if rn_on==1:
                ...
                log_acc_ratio = ptas[n_source][0][rn_on].get_lnlikelihood(np.delete(new_point, range(n_source*7+num_per_psr_params, n_source*7+num_noise_params)))/Ts[j]
        ...
        #turning on -----------------------------------------------------------
            ...
            if rn_on==1:
                ...
                log_acc_ratio = ptas[n_source][1][rn_on].get_lnlikelihood(np.delete(new_point, range(n_source*7+num_per_psr_params, n_source*7+num_noise_params)))/Ts[j]
...
#
###############################################################################
def do_rj_move(n_chain, max_n_source, n_source_prior, ptas, samples, i, Ts,
               a_yes, a_no, freqs, fe, inc_max, psi_max, phase0_max, h_max,
               rj_record, vary_white_noise, include_gwb,
               num_noise_params, num_per_psr_params):
    for j in range(n_chain):
        n_source = int(np.copy(samples[j,i,0]))
        ...
        #adding a signal-------------------------------------------------------
        if n_source==0 or (direction_decide<add_prob and n_source!=max_n_source):
            if j==0:
                rj_record.append(1)
            #alpha = 0.1
            ...
            prior_ext = (ptas[-1][gwb_on][1].params[1].get_pdf(cos_inc) *
                         ptas[-1][gwb_on][1].params[6].get_pdf(psi) *
                         ptas[-1][gwb_on][1].params[5].get_pdf(phase0) *
                         ptas[-1][gwb_on][1].params[4].get_pdf(log10_h))
            ...
            new_source = np.array([np.cos(gw_theta), cos_inc, gw_phi,
                                   log_f_new, log10_h, phase0, psi])
            ...
            #than sum over frequency, but only add half of the first and last bin,
            #because their bin size is half of the others
            ...
            #correction close to edge based on eqs. (40) and (41) of
            #Sambridge et al. Geophys J. Int. (2006) 167, 528-542
            if n_source==0:
                acc_ratio *= 0.5
            ...
            if np.random.uniform()<=acc_ratio:
                ...
                a_no[0,j] += 1
        #removing a signal----------------------------------------------------------
        elif n_source==max_n_source or (direction_decide>add_prob and n_source!=0):
            if j==0:
                rj_record.append(-1)
...
def do_fe_global_jump(n_chain, max_n_source, ptas, samples, i, Ts, a_yes, a_no,
                      freqs, fe, inc_max, psi_max, phase0_max, h_max,
                      vary_white_noise, include_gwb,
                      num_noise_params, num_per_psr_params, Fe_pdet, Fe_alpha,
                      psi_pdf, cos_inc_pdf, phase0_pdf, log_h_pdf):
    #ndim = n_source*7
    ...
    for j in range(n_chain):
        ...
        det_jump = bool(np.random.uniform()<p_det)
        ...
        new_point[source_select*7:(source_select+1)*7] = np.array([np.cos(gw_theta), cos_inc,
                                                                   gw_phi, log_f_new,
                                                                   log10_h, phase0, psi])
        #print(new_point)
        ...
        for k, old_param_fe, new_param, new_param_fe, ext_pdf in zip([1,4,5,6], old_params_fe,
                                                                     new_params, new_params_fe,
                                                                     external_pdfs):
            ...
            #probability that it was put there as deterministic
            #given that it's in a deterministic place
            ...
            if det_new and not det_old: #from non-det to det
                hastings_extra_factor *= prior_old / ( (1-p_det)*prior_new +
                                                       p_det*prior_det_new*ext_pdf(new_param) )
            elif not det_new and det_old: #from det to non-det
                hastings_extra_factor *= ( (1-p_det)*prior_old +
                                           p_det*prior_det_old*ext_pdf(old_param) ) / prior_new
            elif det_new and det_old: #from det to det
                hastings_extra_factor *= ( ( (1-p_det)*prior_old +
                                             p_det*prior_det_old*ext_pdf(old_param) ) /
                                           ( (1-p_det)*prior_new +
                                             p_det*prior_det_new*ext_pdf(new_param) ) )
...
###############################################################################
#
#REGULAR JUMP (JUMPING ALONG EIGENDIRECTIONS IN CW, GWB AND RN PARAMETERS)
#
###############################################################################

def regular_jump(n_chain, max_n_source, ptas, samples, i, Ts, a_yes, a_no,
                 eig, eig_gwb_rn, include_gwb, num_noise_params, num_per_psr_params,
                 vary_rn, vary_rn_gamma, vary_gwb_gamma):
    for j in range(n_chain):
        n_source = int(np.copy(samples[j,i,0]))
        ...
        if what_to_vary == 'CW':
            ...
            jump = np.array([jump_1source[int(i-source_select*7)]
                             if source_select*7 <= i < (source_select+1)*7
                             else 0.0 for i in range(samples_current.size)])
            #print('cw')
            #print(jump)
            ...
            if gwb_on==1 and rn_on ==1: #both gwb and rn are on
                jump = np.array([jump_gwb[int(i-n_source*7-num_per_psr_params)]
                                 if (n_source*7+num_per_psr_params <=
                                     i < n_source*7+num_noise_params+num_gwb_params)
                                 else 0.0 for i in range(samples_current.size)])
...
###############################################################################
#
#NOISE JUMP (JUMPING ALONG EIGENDIRECTIONS IN WHITE NOISE PARAMETERS)
#
###############################################################################

def noise_jump(n_chain, max_n_source, ptas, samples, i, Ts, a_yes, a_no,
               eig_per_psr, include_gwb, num_noise_params, num_per_psr_params,
               vary_white_noise):
    for j in range(n_chain):
        n_source = int(np.copy(samples[j,i,0]))
        ...
        jump = np.array([jump_wn[int(i-n_source*7)]
                         if n_source*7 <= i < n_source*7+eig_per_psr.shape[1]
                         else 0.0 for i in range(samples_current.size)])
        #if j==0: print('noise')
        #if j==0:
        #    print(jump)
...
#
###############################################################################
def do_pt_swap(n_chain, max_n_source, ptas, samples, i, Ts, a_yes, a_no,
               swap_record, vary_white_noise, include_gwb,
               num_noise_params, num_per_psr_params):
...
def get_fisher_eigenvectors(params, pta, T_chain=1, epsilon=1e-4, n_source=1,
                            dim=7, offset=0, use_prior=False):
    ...
    for k in range(n_source):
        ...
        #TODO define exception type which catches what we need
        except:
...
def make_fe_global_proposal(fe_func, f_min=1e-9, f_max=1e-7, n_freq=400,
    ...
    for i, f in enumerate(freqs):
        ...
        if maximized_parameters:
            (m[i,:], inc_max[i,:], psi_max[i,:],
             phase0_max[i,:], h_max[i,:]) = fe_func(f, np.array(hp.pix2ang(NSIDE, idx)),
...
def transdim_postprocess(samples, pulsars=None, ptas=None, separation_method='match',
                         f_tol=0.1, match_tol=0.5, chisq_tol=9.0, max_n_source=10,
                         status_every=1000, include_gwb=False):
    N = samples.shape[0]
    ...
    if separation_method=='freq':
                    ...
                    if new:
                        ...
                        for diff, idx in f_diff:
                            if diff < min_diff:
                                min_diff = diff
                                freq_idx = idx
                        freqs[freq_idx] += ( (f - freqs[freq_idx]) /
                                             (len(source_on_idxs[freq_idx]) + 1) )
                        sample_dict[freq_idx] = np.append(sample_dict[freq_idx],
                                                          np.array([list(samples[i,1+7*j:1+7*(j+1)]),]),
                                                          axis=0)
        ...
        found_all = False
        flagged_ij = []
        #dict to keep delta_L values
        #so we don't need to calculate them every time we loop through the samples
        delta_L_dict = {}
        logL_dict = {}
        flagged_is = []
        while not found_all:
            ...
            #Find the source within the highest likelihood sample
            #that has the highest delta likelihood when the source is removed
            max_delta_L = 0.0
            ...
            for remove_index in range(n_source):
                if (maxL_i, remove_index) not in flagged_ij:
                    ...
                    if (maxL_i, remove_index) in delta_L_dict:
                        ...
                        new_point = np.delete(samples_current,
                                              range(remove_index*7,(remove_index+1)*7))
                        delta_L = (ptas[n_source][gwb_on][1].get_lnlikelihood(samples_current) -
                                   ptas[(n_source-1)][gwb_on][1].get_lnlikelihood(new_point))
...
def get_similarity_matrix(pta, params_list, noise_param_dict=None):
    ...
    Ts = pta.get_basis()
    Nvecs = pta.get_ndiag([])
    Nmats = [ Fe_statistic.make_Nmat(phiinv, TNT, Nvec, T) for phiinv, TNT, Nvec, T
              in zip(phiinvs, TNTs, Nvecs, Ts)]
...
def get_ptas(pulsars, vary_white_noise=True, include_equad_ecorr=False,
             wn_backend_selection=False, noisedict_file=None,
             include_rn=True, include_per_psr_rn=False, vary_rn=True, vary_per_psr_rn=False,
             include_gwb=True, max_n_source=1, efac_start=1.0,
             rn_amp_prior='uniform', rn_log_amp_range=[-18,-11],
             per_psr_rn_amp_prior='uniform', per_psr_rn_log_amp_range=[-18,-11],
             rn_params=[-13.0,1.0],
             gwb_amp_prior='uniform', gwb_log_amp_range=[-18,-11],
             n_comp_common=30, n_comp_per_psr_rn=30,
             vary_gwb_gamma=True, vary_rn_gamma=True,
             cw_amp_prior='uniform', cw_log_amp_range=[-18,-11],
             cw_f_range=[3.5e-9,1e-7], include_psr_term=False, prior_recovery=False):
    ...
    if include_per_psr_rn:
        ...
        if vary_per_psr_rn:
            if per_psr_rn_amp_prior == 'uniform':
                log10_A = parameter.LinearExp(per_psr_rn_log_amp_range[0],
                                              per_psr_rn_log_amp_range[1])
            elif per_psr_rn_amp_prior == 'log-uniform':
                log10_A = parameter.Uniform(per_psr_rn_log_amp_range[0],
                                            per_psr_rn_log_amp_range[1])
